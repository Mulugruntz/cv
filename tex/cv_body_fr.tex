...
%				\item[+] 
		...
		\begin{itemize}
			\item[] Cr\'eation de \ifisanon www....com \textit{(Anonyme)} \else www.qualigraphe.com \fi et de son CMS modulaire sur mesure. Visuels pourvus par le client. %cr\'eation d'un petit CMS avec quelques modules~: un moteur de news, un moteur de courriels ainsi que des modules requis par le client (le site doit pouvoir \^etre compl\'et\'e avec MS Excel). La charte graphique a \'et\'e pourvue par le client.
		\end{itemize}
		...
		\begin{itemize}
			...
			% L'id\'ee originale, la complexit\'e du code et la preuve de non-viabilit\'e dans un environnement bon march\'e m'ont valu un double A.
		\end{itemize}
		\item[] F\'evrier - mai 2009 \tabto{5cm} Stage d'observation en milieu scolaire (Technologie au coll\`ege)\hfill Montauban -- France
%		\begin{itemize}
%			\item[] \ifisanon \else \textit{Coll\`ege Ingres}. \fi Assistance au professeur et aux \'el\`eves. J'ai eu l'honneur de pouvoir donner le dernier cours. 
%		\end{itemize}
		\item[] Juin - ao\^ut 2008 \tabto{5cm} Stage (PHP, JS, HTML/CSS) \hfill Toulouse -- France
%		\begin{itemize}
%			\item[] Cr\'eation de \ifisanon www....fr \textit{(Anonyme)} \else www.amar-abraham.fr \fi , un site web avec Back-end pour un designer de mode. Stage de fin de DUT.
%		\end{itemize}
		\item[] Octobre - Novembre 2007 \tabto{5cm} Maintenance Informatique \hfill Toulouse -- France
%		\begin{itemize}
%			\item[] Permanence informatique sur les sites de Ponsan et de Rangueil concernant la partie logicielle des installations du \textbf{CROUS de Toulouse}, c\^ot\'e client. J'ai \'et\'e recommand\'e par le directeur du campus de Ponsan.
%		\end{itemize}
	\end{itemize}
